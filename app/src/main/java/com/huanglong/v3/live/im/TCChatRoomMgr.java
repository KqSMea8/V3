package com.huanglong.v3.live.im;
...
public class TCChatRoomMgr implements TIMMessageListener {
    ...
    /**
     * @param param zan / cancel_zan
     */
    public void sendPraiseMessage(String param) {
        sendMessage(TCConstants.IMCMD_PRAISE, param);
    }
    ...
    public void sendLiveGift(String msg) {
        sendMessage(TCConstants.LIVE_GIFT, msg);
    }
    ...
    public void createGroup() {
        ...
        checkLoginState(new TCLoginMgr.TCLoginCallback() {
            ...
            public void onSuccess() {
                ...
                //用户登录，创建直播间
                GroupManagerPresenter.createAVChatRoomGroup("TVShow", new TIMValueCallBack<String>() {
                    @Override
                    public void onError(int code, String msg) {
                        LogUtil.e("create av group failed. code: " + code + " errmsg: " + msg);
                    ...
                    public void onSuccess(String roomId) {
                        LogUtil.i("create av group succ, groupId:" + roomId);
                        mRoomId = roomId;
    ...
    public void deleteGroup() {
        ...
        GroupManagerPresenter.deleteGroup(mRoomId, new TIMCallBack() {
            @Override
            public void onError(int code, String msg) {
                LogUtil.e("delete av group failed. code: " + code + " errmsg: " + msg);
//                if (null != mTCChatRoomListener)
//                    mTCChatRoomListener.onQuitGroupCallback(code, msg);
            ...
            @Override
            public void onSuccess() {
                LogUtil.i("delete av group succ. groupid: " + mRoomId);
//                if (null != mTCChatRoomListener)
//                    mTCChatRoomListener.onQuitGroupCallback(0, mRoomId);
    ...
    public void quitGroup(final String roomId) {
        GroupManagerPresenter.quitGroup(roomId, new TIMCallBack() {
            @Override
            public void onError(int i, String s) {
                LogUtil.e("quitGroup failed, code:" + i + ",msg:" + s);
//                    if(null != mTCChatRoomListener) {
...
//                    }
            ...
            @Override
            public void onSuccess() {
                LogUtil.i("quitGroup success, groupid:" + roomId);
                mTCChatRoomListener = null;
                mGroupConversation = null;
            }
        });
        ...
        checkLoginState(new TCLoginMgr.TCLoginCallback() {
            ...
            public void onSuccess() {
                ...
                TIMGroupManager.getInstance().applyJoinGroup(roomId, "", new TIMCallBack() {
                    @Override
                    public void onError(int i, String s) {
                        LogUtil.e("joingroup failed, code:" + i + ",msg:" + s);
                        mRoomId = null;
                        if (null != mTCChatRoomListener)
                            mTCChatRoomListener.onJoinGroupCallback(i, s);
                        else
                            LogUtil.e("mPlayerListener not init");
                    }

                    @Override
                    public void onSuccess() {
                        LogUtil.i("joingroup success, groupid:" + roomId);
                        mGroupConversation = TIMManager.getInstance().getConversation(TIMConversationType.Group, roomId);
                        //加入群组成功后，发送加入消息
                        ...
                        if (null != mTCChatRoomListener) {
                            ...
                            LogUtil.i("mPlayerListener not init");
                        }
                    }
                });
            }
            ...
            @Override
            public void onFailure(int code, String msg) {
                LogUtil.e("relogin fail. code: " + code + " errmsg: " + msg);
                TCLoginMgr.getInstance().removeTCLoginCallback();
                if (null != mTCChatRoomListener)
                    mTCChatRoomListener.onJoinGroupCallback(TCConstants.NO_LOGIN_CACHE, "no login cache, user has been kicked out");
    ...
    public boolean onNewMessages(List<TIMMessage> list) {
        LogUtil.e("----new msg size " + list.size());
        parseIMMessage(list);
        return false;
    }
...
//            bMore = false;
    ...
    private void sendTIMMessage(TIMMessage msg, TIMValueCallBack<TIMMessage> timValueCallBack) {
        if (mGroupConversation != null) {
            TIMTextElem timTextElem = (TIMTextElem) msg.getElement(0);
            String text = timTextElem.getText();
            LogUtil.e("live curr msg " + text);
    ...
    private void sendMessage(int cmd, String param, TIMValueCallBack<TIMMessage> timValueCallBack) {
        ...
        try {
            ...
            sendJson.put("userId", UserInfoUtils.getUid());
            sendJson.put("nickName", UserInfoUtils.getNickName());
            sendJson.put("headPic", UserInfoUtils.getAvatar());
...
//        if (cmd == TCConstants.IMCMD_PAILN_TEXT) {
    ...
    private void handleCustomTextMsg(TIMElem elem, TIMUserProfile timUserProfile) {
        try {
            ...
            LogUtil.e("chatroom cumstom msg  " + jsonString);
