...
public class GroupInfo implements Observer {
    ...
    public static String getTypeName(String type) {
        if (type.equals(GroupInfo.publicGroup)) {
            return V3Application.getInstance().getString(R.string.public_group);
        } else if (type.equals(GroupInfo.privateGroup)) {
            return V3Application.getInstance().getString(R.string.discuss_group);
        } else if (type.equals(GroupInfo.chatRoom)) {
            return V3Application.getInstance().getString(R.string.chatroom);
        }
        return "";
    }
     ...
     * @param identify 群id
     */
    public String getGroupAvatar(String identify) {
        for (String key : groups.keySet()) {
            for (GroupProfile item : groups.get(key)) {
                if (item.getIdentify().equals(identify)) return item.getAvatarUrl();
            }
        }
        return "";
    }
