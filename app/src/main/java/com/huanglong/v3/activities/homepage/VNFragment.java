...
public class VNFragment extends BaseFragment {

    @ViewInject(R.id.activity_video_nearby_tab)
    private TabLayout tab_video;
    @ViewInject(R.id.activity_video_nearby_view_pager)
    ...
    protected View getContentView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_video_nearby_tab, container, false);
        return view;
    }
    ...
    private void initTabItem(List<LiveClassBean> liveClassBeans) {
        ...
        for (int i = 0; i < liveClassBeans.size(); i++) {
            ...
            fragments.add(new VNCFragment(liveClassBeans.get(i).getId()));
        }
    ...
    public void refresh() {
        if (isCreate) {
            for (int i = 0; i < fragments.size(); i++) {
                VNCFragment vncFragment = (VNCFragment) fragments.get(i);
                boolean isCreate = vncFragment.isCreate;
                if (isCreate) {
                    vncFragment.onRefresh();
                }
            }
        }
    }
