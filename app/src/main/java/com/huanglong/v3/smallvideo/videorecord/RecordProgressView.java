...
//        mSpaceColor = getResources().getColor(R.color.white);
        ...
        mBackgroundColor = ContextCompat.getColor(context, R.color.record_progress_bg);
        mRecordColor = ContextCompat.getColor(context, R.color.record_progress);
        mPendingColor = ContextCompat.getColor(context, R.color.record_progress_pending);
        mSpaceColor = ContextCompat.getColor(context, R.color.white);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (ClipInfo clipInfo : mClipInfoList) {
            ...
            switch (clipInfo.clipType) {
                case ClipInfo.CLIP_TYPE_SPACE:
                    canvas.drawRect(totalWidth - getResources().getDimension(R.dimen.bin_1_dip), 0f, newWidth, getHeight(), mSpacePaint);
                    break;
        ...
        if (lastTotalProgress + mCurClipInfo.progress < mMinDuration) {
            canvas.drawRect(mMinDuration / (float) mMaxDuration * getWidth(), 0f,
                    mMinDuration / (float) mMaxDuration * getWidth() + getResources().getDimension(R.dimen.bin_2_dip), getHeight(), mSpacePaint);
        }
        if (isCursorShow || isInProgress) {
            canvas.drawRect(totalWidth, 0f, totalWidth + getResources().getDimension(R.dimen.bin_2_dip), getHeight(), mSpacePaint);
        }
    }
